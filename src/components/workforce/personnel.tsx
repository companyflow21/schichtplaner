...
export function Personnel({ id }: { id: string }) {
  ...
  return <Card className="p-5"><h2 className="text-[16px] font-semibold tracking-[-0.02em]">Beschäftigung und Qualifikationen</h2><form key={JSON.stringify(p)} className="grid gap-4 sm:grid-cols-2" onSubmit={async e => { e.preventDefault(); const f = new FormData(e.currentTarget); await action.mutateAsync({ url: "/api/employees/" + id, method: "PATCH", data: { position: f.get("position"), employmentType: f.get("employmentType"), targetHoursPerWeek: Number(f.get("hours")), qualifications: String(f.get("qualifications")).split(",").map(s => s.trim()).filter(Boolean), isActive: f.get("active") === "true" } }).catch(() => {}); }}>
